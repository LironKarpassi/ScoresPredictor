...
const ScorePredictor = () => {
  ...
  const handleSelect = (e) => {
    ...
    const selectedTeam = teams.find((teams) => {
      return teams.name === e.target.value;
    });
  ...
  useEffect(() => {
    setTeams([
      { id: "West Ham United", name: "West Ham United", img: West_Ham_United },
      {
        id: "Aston Villa",
        name: "Aston Villa",
        img: Aston_Villa,
  ...
  const [selectdImg1, setselectdImg1] = useState("");
  const [selectdImg2, setselectdImg2] = useState("");
  ...
  const getData = async () => {
    if (
      optionValue === "Click to see options" ||
      optionValue1 === "Click to see options" ||
      optionValue1 === "" ||
      optionValue === ""
    ) {
    ...
    setTimeout(
      () =>
        fetch("https://f5g8f09fqf.execute-api.us-east-1.amazonaws.com/beta", {
          method: "POST",
          headers: {
  ...
  return (
    <div className="main">
      <h1 className="titleofpage">
        Which Teams Are You Insterested To Predict?
      </h1>
      <div className="group-container">

      <div className="group1">
        <div className="img-container">
          {selectdImg1 && <img src={selectdImg1} className="logo" />}
        </div>
        <Dropdown
      ...
      <div className="group2">
        <div className="img-container">
          {selectdImg2 && <img src={selectdImg2} className="logo" />}
        </div>
        <Dropdown
          className="dropdowncss"
          ...
          content={
            <React.Fragment>
              {result.startsWith("Error") ? (
                <b>
                  <h1 className="TitleOfRES">{result}</h1>
                </b>
              ) : (
                <React.Fragment>
                  <b>
                  ...
                  <p>
                    There Is A{" "}
                    {result.substring(1).substring(0, 3)} <>That The Home Team Is Going To Win</>
                    <p>The Rate is : {result.substring(1).substring(4, 7)}</p>
                    <p>{result.substring(1).substring(9, 38)}</p>
